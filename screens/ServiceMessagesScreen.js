...
const ServiceMessagesScreen = () => {
  ...
  const fetchMessages = async () => {
    try {
      ...
      // Filter only Service category messages
      const serviceMessages = res.data.messages.filter(
        (msg) => msg.staffCategory.toLowerCase() === "service"
      );
  ...
  const renderItem = ({ item }) => (
    <TouchableOpacity
      ...
      onPress={() => navigation.navigate("ServiceChat", { userId: item.userId, userName: item.userName })}
    >
...
const styles = StyleSheet.create({
  ...
  messageCard: { padding: 12, borderBottomWidth: 1, borderBottomColor: "#ccc" },
